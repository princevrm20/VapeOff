...
def patient_order_generated(request):
    print(request)
    print("patient_order_generated")
    print(list(request.GET['app_id']))
    Appointment.objects.filter(id=request.GET['app_id']).update(payment_status="VALID")

    # app = Appointment.objects.get(pk=request.GET['app_id'])
    # print(app.payment_status)
    # newapp = Appointment(payment_status="completed", instance = app)
    # newapp.save()


    return render(request, 'order-generated.html')
...
def home(request):
    ...
    if request.method == "POST":
        ...
        client = razorpay.Client(
            auth=("rzp_test_Jgpun0kmjvg32x", "uuZtQ9HMhBHQJMH6tGwYOoI6"))
...
def success(request):
    ...
    #     book_form.save()
    # app = Appointment.objects.get(pk=request.app_id)
    # print(app.appointment_status)
    # newapp = Appointment(appointment_status, instance = app)
    # newapp.save()

    # Appointment.objects.filter(id=request.app_id).update(id=5)
    # Appointment.save()
